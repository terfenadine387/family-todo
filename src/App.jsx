...
function nthWeekdayOfMonth(date) { return Math.ceil(date.getDate() / 7) - 1; }
...
function todoOccursOn(todo, dateStr) {
  ...
  if (t === "weekly") return (todo.weekdays || []).includes(weekdayIndex(date));
...
function getRepeatLabel(todo) {
  ...
  if (t === "monthly_weekday") {
    return `毎月 ${MONTH_WEEK_POS[todo.monthWeekPos ?? 0]}${WEEKDAYS_JP[todo.monthWeekDay ?? 0]}曜`;
  }
...
function isRecurring(todo) { return todo.repeat !== "once"; }
...
// ── MemberIcon ─────────────────────────────────────────────
function MemberIcon({ member, size = 40, done = false }) {
  if (done) return <span style={{ fontSize: size * 0.5 }}>✓</span>;
  if (member.imageUrl) {
    return <img src={member.imageUrl} style={{ width:"100%", height:"100%", objectFit:"cover" }} />;
  }
  return <span style={{ fontSize: size * 0.5 }}>{member.emoji || "👤"}</span>;
}

// ── MemberSelect ───────────────────────────────────────────
function MemberSelect({ members, onSelect, onAdd, onDelete, onImageChange }) {
  ...
  const handleAdd = async () => {
    ...
    setNewName(""); setNewEmoji("👤"); setNewColor("#60a5fa");
    setNewImageUrl(""); setShowAddForm(false);
  };
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize:14, color:"#64748b", marginBottom:32 }}>だれですか？</div>
        ...
        {members.map(m => (
          <div key={m.id}>
            ...
            <button onClick={() => handleSelect(m.id)} style={{
              ...
              borderRadius: deleteConfirm === m.id ? "20px 20px 0 0" : "20px 20px 0 0",
            ...
            <div style={{
              ...
              {deleteConfirm === m.id ? (
                <>
                  <div style={{ flex:1, padding:"10px 0", background:"#1e293b", textAlign:"center", fontSize:12, color:"#94a3b8", display:"flex", alignItems:"center", justifyContent:"center" }}>
                    本当に削除しますか？
                  </div>
          ...
          <div style={{ background:"#1e293b", borderRadius:20, padding:16, border:"1px solid #334155" }}>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:6, flex:1 }}>
                <label style={{
                  ...
                  <input type="file" accept="image/*" style={{ display:"none" }}
                    onChange={async (e) => {
                      ...
                      setUploading(true);
                      try {
                        const storageRef = ref(storage, `members/new_${Date.now()}`);
                ...
                {newImageUrl && (
                  <button onClick={() => { setNewImageUrl(""); setNewEmoji("👤"); }} style={{
                    ...
                    border:"1px solid #334155", color:"#ff4757", fontSize:11, cursor:"pointer"
                  }}>画像を削除</button>
                )}
              </div>
            </div>
            ...
            <div style={{ display:"flex", gap:8, marginBottom:16 }}>
              {COLOR_OPTIONS.map(c => (
                <div key={c} onClick={() => setNewColor(c)} style={{
                  ...
                  border:`3px solid ${newColor===c?"#fff":"transparent"}`, transition:"border 0.15s"
                }}/>
              ))}
            </div>
            ...
            <div style={{
              ...
              <div style={{
                ...
                background:newColor+"33", display:"flex", alignItems:"center", justifyContent:"center", fontSize:24
              }}>
                {newImageUrl
                  ? <img src={newImageUrl} style={{ width:"100%", height:"100%", objectFit:"cover" }}/>
                  : newEmoji
...
// ── Main App ───────────────────────────────────────────────
function emptyDraft(selectedDate, firstMemberId = "") {
  return {
    title:"", memo:"", assignee: firstMemberId,
...
export default function FamilyTodo() {
  ...
  useEffect(() => {
    ...
    return () => { unsubTodos(); unsubNotifs(); unsubMembers(); };
  }, []);
  ...
  useEffect(() => {
    ...
    async function registerToken() {
      try {
        ...
        if (token) {
          await setDoc(doc(db, "members", currentUser), { fcmToken: token, updatedAt: serverTimestamp() }, { merge: true });
        }
      } catch (err) { console.warn("FCM登録エラー:", err); }
    }
    registerToken();
  }, [currentUser]);
  ...
  function isCompleted(todo) { return (todo.completedDates||[]).includes(selectedDate); }

  async function toggleComplete(todo) {
    const dates = todo.completedDates || [];
    const already = dates.includes(selectedDate);
    const next = already ? dates.filter(d => d !== selectedDate) : [...dates, selectedDate];
    const completedTimes = { ...(todo.completedTimes||{}) };
    if (!already) {
      const now = new Date();
      completedTimes[selectedDate] = `${String(now.getHours()).padStart(2,"0")}:${String(now.getMinutes()).padStart(2,"0")}`;
    } else {
      delete completedTimes[selectedDate];
    }
    await updateDoc(doc(db, "todos", todo.id), { completedDates: next, completedTimes });
    if (!already) {
      await addDoc(collection(db, "notifications"), {
        type:"complete", from:currentUser, todoTitle:todo.title,
        time:completedTimes[selectedDate], read:false, createdAt:serverTimestamp(),
      });
  ...
  function openAdd() {
    setEditingId(null);
    setDraft(emptyDraft(selectedDate, members[0]?.id || ""));
    setShowModal(true);
  }
  ...
  async function handleDelete(todo) {
    if (isRecurring(todo)) {
      setScopeDialog({
        action:"delete", todo,
        onThisDay: async () => {
          await updateDoc(doc(db, "todos", todo.id), { skippedDates:[...(todo.skippedDates||[]), selectedDate] });
          setScopeDialog(null);
          showToast(`「${todo.title}」この日をスキップしました`);
        },
        onFromHere: async () => {
          const prev = new Date(parseYMD(selectedDate)); prev.setDate(prev.getDate()-1);
  ...
  async function addMember({ name, emoji, imageUrl, color }) {
    const id = `${name}_${Date.now()}`;
    await setDoc(doc(db, "members", id), {
      name, emoji: emoji||"", imageUrl: imageUrl||"", color,
      order: members.length, createdAt: serverTimestamp(),
    });
  }
  ...
  async function changeMemberImage(memberId, file) {
    try {
      const storageRef = ref(storage, `members/${memberId}_${Date.now()}`);
      await uploadBytes(storageRef, file);
      const url = await getDownloadURL(storageRef);
      await updateDoc(doc(db, "members", memberId), { imageUrl: url });
  ...
  async function save() {
    ...
    if (editingId) {
      ...
      if (scope === "thisDay") {
        ...
        await updateDoc(doc(db, "todos", id), { skippedDates:[...(orig.skippedDates||[]), date] });
        await addDoc(collection(db, "todos"), { ...draft, startDate:date, endDate:date, repeat:"once", completedDates:[], completedTimes:{}, skippedDates:[], createdAt:serverTimestamp() });
      } else if (scope === "fromHere") {
        const prev = new Date(parseYMD(date)); prev.setDate(prev.getDate()-1);
        await updateDoc(doc(db, "todos", id), { endDate:toYMD(prev) });
        await addDoc(collection(db, "todos"), { ...draft, completedDates:[], completedTimes:{}, skippedDates:[], createdAt:serverTimestamp() });
      } else {
        const orig = todos.find(t => t.id === id);
        await updateDoc(doc(db, "todos", id), { ...draft, completedDates:orig.completedDates||[], completedTimes:orig.completedTimes||{}, skippedDates:orig.skippedDates||[] });
      }
  ...
  if (!currentUser) {
    return (
      <MemberSelect
        members={members}
        onSelect={(id) => { localStorage.setItem("familyTodoUser", id); setCurrentUser(id); }}
        onAdd={addMember}
        onDelete={deleteMember}
        onImageChange={changeMemberImage}
      />
    );
  }
  ...
  return (
    <div style={{
      ...
      <div style={{ padding:"20px 20px 0", display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
        <div>
          <div style={{ fontSize:32, fontWeight:800, letterSpacing:-1, color:"#f1f5f9" }}>{M}月{D}日</div>
          <div style={{ fontSize:12, color:"#64748b", marginTop:2 }}>{WEEKDAYS_JP[weekdayIndex(selDateObj)]}曜日</div>
        </div>
        <div style={{ display:"flex", gap:10, alignItems:"center" }}>
          ...
          <button onClick={() => { localStorage.removeItem("familyTodoUser"); setCurrentUser(null); }} style={{
            display:"flex", alignItems:"center", gap:6,
            background:memberObj.color+"22", border:`1px solid ${memberObj.color}44`,
            borderRadius:20, padding:"4px 12px 4px 4px", cursor:"pointer",
            color:memberObj.color, fontSize:13, fontWeight:600,
          }}>
            <div style={{ width:28, height:28, borderRadius:8, overflow:"hidden", background:memberObj.color+"33", display:"flex", alignItems:"center", justifyContent:"center", fontSize:16 }}>
              {memberObj.imageUrl
                ? <img src={memberObj.imageUrl} style={{ width:"100%", height:"100%", objectFit:"cover" }}/>
                : memberObj.emoji || "👤"
              }
            </div>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:12, padding:"14px 14px" }}
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  ...
                  <div style={{ display:"flex", alignItems:"center", gap:8, marginTop:4, flexWrap:"wrap" }}>
                    ...
                    {memberFilter === "all" && (
                      <span style={{ fontSize:11, color:"#64748b" }}>{am.name}</span>
                    )}
                  </div>
        ...
        <div style={{ position:"fixed", inset:0, zIndex:200, background:"#0008" }}
          ...
          <div onClick={e => e.stopPropagation()} style={{
            ...
            <div style={{ flex:1, overflowY:"auto" }}>
              ...
              {notifications.filter(n => n.from !== currentUser).map(n => {
                ...
                return (
                  <div key={n.id} style={{ padding:"12px 20px", background:n.read?"transparent":"#1e293b", borderBottom:"1px solid #1e293b22", display:"flex", gap:10, alignItems:"flex-start" }}>
                    <div style={{ width:32, height:32, borderRadius:8, overflow:"hidden", background:from?.color+"33"||"#334155", display:"flex", alignItems:"center", justifyContent:"center", fontSize:18, flexShrink:0 }}>
                      {from?.imageUrl
                        ? <img src={from.imageUrl} style={{ width:"100%", height:"100%", objectFit:"cover" }}/>
                        : from?.emoji||"👤"
                      }
                    </div>
                    <div style={{ flex:1 }}>
                      <div style={{ fontSize:13, marginBottom:2 }}>
                        <span style={{ color:from?.color||"#94a3b8", fontWeight:600 }}>{from?.name||"?"}</span>
                ...
                <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                  {members.map(m => (
                    <div key={m.id} onClick={() => setDraft(d=>({...d,assignee:m.id}))} style={{
                      ...
                      <div style={{ width:24, height:24, borderRadius:6, overflow:"hidden", background:m.color+"33", display:"flex", alignItems:"center", justifyContent:"center", fontSize:14 }}>
                        {m.imageUrl
                          ? <img src={m.imageUrl} style={{ width:"100%", height:"100%", objectFit:"cover" }}/>
                          : m.emoji||"👤"
                        }
                ...
                <div style={{ background:"#1e293b", borderRadius:12, padding:"12px 14px", border:"1px solid #334155" }}>
                  <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:draft.notifyEnabled?12:0 }}>
                    ...
                    <div onClick={() => setDraft(d=>({...d,notifyEnabled:!d.notifyEnabled}))} style={{
                      ...
                      <div style={{
                        position:"absolute", top:3, left:draft.notifyEnabled?21:3,
